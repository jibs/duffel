...
func TestDeleteRecursive_Directory(t *testing.T) {
	store, err := NewStore(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}

	if err := store.Write("project/notes/plan.md", []byte("x")); err != nil {
		t.Fatal(err)
	}
	if err := store.DeleteRecursive("project"); err != nil {
		t.Fatalf("DeleteRecursive project: %v", err)
	}

	_, err = store.List("project", false)
	if !errors.Is(err, ErrNotFound) {
		t.Fatalf("List project after recursive delete err = %v, want ErrNotFound", err)
	}
}

func TestDeleteRecursive_RejectsRoot(t *testing.T) {
	store, err := NewStore(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, path := range []string{"", "/", "."} {
		t.Run(path, func(t *testing.T) {
			err := store.DeleteRecursive(path)
			if !errors.Is(err, ErrPathTraversal) {
				t.Fatalf("DeleteRecursive(%q) err = %v, want ErrPathTraversal", path, err)
			}
		})
	}
}
