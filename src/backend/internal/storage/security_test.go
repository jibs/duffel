...
// to the same destination cannot silently overwrite each other, even when they
// come from separate Store instances sharing the same root.
//
// Root cause: Move() originally guarded only with an in-process mutex and a
// fileExists(dst) check before os.Rename(), leaving a cross-instance TOCTOU.
//
// Fix: serialize on a destination-specific filesystem lock, not just memory.
func TestSecurity_MoveRaceCondition(t *testing.T) {
	...
	for range iterations {
		...
		_ = storeA.Delete(dest)
		_ = storeA.Delete(nameA)
		_ = storeA.Delete(nameB)

		if err := storeA.Write(nameA, []byte("content-A")); err != nil {
			t.Fatal(err)
		}
		if err := storeA.Write(nameB, []byte("content-B")); err != nil {
			t.Fatal(err)
		}
		...
		go func() {
			defer wg.Done()
			<-start
			errA = storeA.Move(nameA, dest)
		}()
		go func() {
			defer wg.Done()
			<-start
			errB = storeB.Move(nameB, dest)
		}()
		close(start)
		wg.Wait()
