...
func TestParseSearchFields(t *testing.T) {
	...
	t.Run("valid with dedupe and spaces", func(t *testing.T) {
		fields, err := parseSearchFields(" path ,title,path, modified_at, line, context, explain ")
		if err != nil {
			t.Fatalf("parseSearchFields returned error: %v", err)
		}
		want := []string{"path", "title", "modified_at", "line", "context", "explain"}
...
func TestProjectSearchResults(t *testing.T) {
	...
	got := projectSearchResults(results, []string{"path", "title", "line", "context", "explain"})
	if len(got) != 1 {
		t.Fatalf("len(got) = %d, want 1", len(got))
	}
	if len(got[0]) != 5 {
		t.Fatalf("len(got[0]) = %d, want 5", len(got[0]))
	}
	...
	if got[0]["line"] != 17 {
		t.Fatalf("line = %v, want 17", got[0]["line"])
	}
	if got[0]["context"] != "Project notes" {
		t.Fatalf("context = %v, want Project notes", got[0]["context"])
	}
