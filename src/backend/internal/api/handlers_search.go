...
func parseSearchFields(raw string) ([]string, error) {
	...
	for _, part := range parts {
		...
		if _, ok := allowed[field]; !ok {
			return nil, fmt.Errorf("invalid fields parameter: unknown field %q (allowed: path,title,snippet,score,modified_at,line,context,explain)", field)
		}
...
func projectSearchResults(results []search.Result, fields []string) []map[string]any {
	...
	for _, result := range results {
		...
		for _, field := range fields {
			...
			case "line":
				item[field] = result.Line
			case "context":
				item[field] = result.Context
			case "explain":
