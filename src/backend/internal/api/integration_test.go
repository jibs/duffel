...
func setupTestServerWithHook(t *testing.T, onContentChanged func()) (*httptest.Server, *storage.Store) {
	t.Helper()
	t.Setenv("DUFFEL_AUTH_ENABLED", "0")
	dir := t.TempDir()
...
func TestResponsesDisableBrowserCaching(t *testing.T) {
...
func TestReadHTMLFileIncludesHTMLKind(t *testing.T) {
	srv, _ := setupTestServer(t)
	defer srv.Close()

	req, _ := http.NewRequest("PUT", srv.URL+"/api/fs/pages/home.html", strings.NewReader(`{"content":"<h1>Home</h1><a href=\"next.html\">Next</a>"}`))
	req.Header.Set("Content-Type", "application/json")
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
	...
	resp, err = http.Get(srv.URL + "/api/fs/pages/home.html")
	if err != nil {
		t.Fatal(err)
	}
	defer resp.Body.Close()
	...
	if payload["kind"] != "html" {
		t.Fatalf("kind = %v, want html", payload["kind"])
	}
}

func TestListIncludesHTMLKind(t *testing.T) {
	srv, store := setupTestServer(t)
	defer srv.Close()

	if err := store.Write("pages/home.html", []byte("<h1>Home</h1>")); err != nil {
		t.Fatalf("seed html: %v", err)
	}

	resp, err := http.Get(srv.URL + "/api/fs/pages")
	if err != nil {
		t.Fatal(err)
	}
	defer resp.Body.Close()
	...
	if payload.Entries[0].Kind != "html" {
		t.Fatalf("entry kind = %q, want html", payload.Entries[0].Kind)
	}
}
...
func TestContentMutationHooks(t *testing.T) {
	...
	t.Run("create directory triggers hook", func(t *testing.T) {
		resp, err := http.Post(srv.URL+"/api/fs/hook-dir", "application/json",
		...
		resp.Body.Close()
		if resp.StatusCode != http.StatusCreated {
			t.Fatalf("create directory status = %d, want 201", resp.StatusCode)
		}
	})
...
func TestRealtimeEventsStreamContentChanges(t *testing.T) {
	srv, _ := setupTestServer(t)
	defer srv.Close()
	...
	putReq, _ := http.NewRequest(http.MethodPut, srv.URL+"/api/fs/live.md", strings.NewReader(`{"content":"live"}`))
	...
	waitForLine(t, lines, "event: content-changed")
}
...
func TestDeleteDirectoryRecursive(t *testing.T) {
	...
	if err := store.Write("project/notes/plan.md", []byte("bye")); err != nil {
		t.Fatal(err)
	}

	req, _ := http.NewRequest("DELETE", srv.URL+"/api/fs/project?recursive=true", nil)
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("DELETE recursive status = %d, want 200", resp.StatusCode)
	}

	resp, err = http.Get(srv.URL + "/api/fs/project")
	if err != nil {
		t.Fatal(err)
	}
	resp.Body.Close()
	if resp.StatusCode != http.StatusNotFound {
		t.Fatalf("GET after recursive delete status = %d, want 404", resp.StatusCode)
	}
}
...
func TestAgentScript(t *testing.T) {
	...
	t.Run("forwarded origin", func(t *testing.T) {
		req, _ := http.NewRequest("GET", srv.URL+"/api/agent/script", nil)
		...
		var body strings.Builder
		io.Copy(&body, resp.Body)
		if !strings.Contains(body.String(), `DUFFEL_URL="${DUFFEL_URL:-https://duffel.tailnet.ts.net}"`) {
			t.Fatalf("script did not use forwarded origin:\n%s", body.String())
		}
	})
}
...
func TestCORSHeaders(t *testing.T) {
	...
	t.Run("forwarded same-origin request gets CORS headers", func(t *testing.T) {
		req, _ := http.NewRequest("GET", srv.URL+"/api/agent/version", nil)
		req.Host = "localhost:4386"
		req.Header.Set("Origin", "https://duffel.tailnet.ts.net")
		req.Header.Set("X-Forwarded-Proto", "https")
		req.Header.Set("X-Forwarded-Host", "duffel.tailnet.ts.net")
		resp, err := http.DefaultClient.Do(req)
		if err != nil {
		...
		resp.Body.Close()
		if got := resp.Header.Get("Access-Control-Allow-Origin"); got != "https://duffel.tailnet.ts.net" {
			t.Fatalf("Access-Control-Allow-Origin = %q, want %q", got, "https://duffel.tailnet.ts.net")
		}
	})
...
func TestCrossOriginWriteBlocked(t *testing.T) {
	...
	t.Run("forwarded same-origin write succeeds", func(t *testing.T) {
		req, _ := http.NewRequest("PUT", srv.URL+"/api/fs/proxied.md", strings.NewReader(`{"content":"ok"}`))
		req.Host = "localhost:4386"
		req.Header.Set("Content-Type", "application/json")
		req.Header.Set("Origin", "https://duffel.tailnet.ts.net")
		req.Header.Set("X-Forwarded-Proto", "https")
