...
function renderResults(data: SearchResult[]): void {
  ...
  data.forEach((result) => {
    ...
    if (result.modified_at) {
      ...
      dateEl.textContent = formatDate(result.modified_at);
...
function isSearchVisible(): boolean {
  return !document.getElementById("view-search")!.classList.contains("hidden");
}

function refreshSearch(): void {
  if (!isSearchVisible() || !currentQuery) return;
  runSearch(currentQuery);
}

export { hideSearch, isSearchVisible, refreshSearch };
